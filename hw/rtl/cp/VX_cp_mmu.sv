...
// model (cmd_processor.cpp cp_translate) uses. A small translation cache
// short-circuits the walk for the repeated per-chunk lookups of a large
// transfer; it is dropped whenever the page table may have changed
// (flush, new SATP) and bypassed in BARE mode. Page-table geometry and
// the PTE fault predicate come from VX_tlb_pkg / match VX_ptw_walker so
// this walker and the core-side PTW translate identically.
module VX_cp_mmu
  import VX_tlb_pkg::*;
  import VX_cp_pkg::*;
(
  ...
  localparam int PTE_SHIFT    = `CLOG2(PTE_SIZE);
...
`endif
  ...
  // the table address (same slice as the core-side walker).
  wire [TLB_PPN_WIDTH-1:0] root_ppn = satp[TLB_PPN_WIDTH-1:0];
  `UNUSED_VAR (satp)
  ...
  function automatic logic [TLB_LEVEL_BITS-1:0] vpn_slice(
    input logic [63:0]                vaddr,
    input logic [TLB_LEVEL_WIDTH-1:0] level
  );
    return vaddr[PAGE_LOG2 + level * TLB_LEVEL_BITS +: TLB_LEVEL_BITS];
  endfunction

  function automatic logic [63:0] level_mask(input logic [TLB_LEVEL_WIDTH-1:0] level);
    return ~((64'd1 << (PAGE_LOG2 + level * TLB_LEVEL_BITS)) - 64'd1);
  endfunction

  // A superpage leaf must have its low PPN bits clear (the page offset
  // covers them) — same test as VX_ptw_walker's pte_misaligned.
  function automatic logic superpage_misaligned(
    input logic [TLB_PPN_WIDTH-1:0]   ppn,
    input logic [TLB_LEVEL_WIDTH-1:0] level
  );
    logic [TLB_PPN_WIDTH-1:0] mask;
    mask = (TLB_PPN_WIDTH'(1) << (level * TLB_LEVEL_BITS)) - TLB_PPN_WIDTH'(1);
    return |(ppn & mask);
  endfunction
  ...
  state_e                     state;
  logic [63:0]                vaddr_r;
  logic [TLB_PPN_WIDTH-1:0]   cur_ppn_r;   // table PPN during the walk
  logic [TLB_LEVEL_WIDTH-1:0] level_r;
  logic [63:0]                paddr_r;

  wire [63:0] pte_addr = (64'(cur_ppn_r) << PAGE_LOG2)
                       | (64'(vpn_slice(vaddr_r, level_r)) << PTE_SHIFT);
  `UNUSED_VAR (pte_addr[PTE_SHIFT-1:0])
  ...
  wire [TLB_FLAGS_WIDTH-1:0] pte_flags = pte_w[TLB_FLAGS_WIDTH-1:0];
  wire [TLB_PPN_WIDTH-1:0]   pte_ppn   = pte_w[10 +: TLB_PPN_WIDTH];
  // U/A/D/G flag bits and the RSW/PBMT/N high PTE bits are not enforced
  // here: the DMA acts for the host, whose ACL is the permission boundary.
  `UNUSED_VAR (pte_w[PTE_BITS-1 : 10+TLB_PPN_WIDTH])
  `UNUSED_VAR (pte_w[9:8])
  wire pte_leaf  = pte_flags[TLB_FLAG_R] | pte_flags[TLB_FLAG_W] | pte_flags[TLB_FLAG_X];
  wire pte_bad   = (~pte_flags[TLB_FLAG_V]) | (~pte_flags[TLB_FLAG_R] & pte_flags[TLB_FLAG_W]);
  wire pte_fault = pte_bad
                || (pte_leaf && superpage_misaligned(pte_ppn, level_r))
                || (~pte_leaf && (level_r == '0));
        ...
        S_IDLE: begin
          if (req_valid) begin
            if (!satp_translate) begin
              ...
              vaddr_r   <= req_vaddr;
              cur_ppn_r <= root_ppn;
              level_r   <= TLB_LEVEL_WIDTH'(TLB_PT_LEVELS - 1);
              state     <= S_FETCH;
            end
          end
        end
          ...
          if (mem_rvalid) begin
            if (pte_fault) begin
              ...
              if (!tc_invalidate) begin
                ...
                tc[tc_rr].vpn   <= vaddr_r[PAGE_LOG2 +: TLB_VPN_WIDTH];
              ...
              cur_ppn_r <= pte_ppn;
              level_r   <= level_r - TLB_LEVEL_WIDTH'(1);
              state     <= S_FETCH;
            end
          end
        end
